...
hs <- makeParamSet(
    ...
    makeIntegerParam("envios",           lower =  7500L, upper = 9000L),
...
params$hyperparametertuning$semilla_azar <- 697157 # Aqui poner la propia semilla, PUEDE ser distinta a la de trainingstrategy
...
fganancia_logistic_lightgbm  <- function(probs, datos)  {
  ...
  #vector de ganancias
  vgan  <- ifelse(vpesos == 1.0000002, params$hyperparametertuning$POS_ganancia, 
                   ifelse(vpesos == 1.0000001, params$hyperparametertuning$NEG_ganancia, 
                           params$hyperparametertuning$NEG_ganancia / params$trainingstrategy$undersampling))
...
EstimarGanancia_lightgbm  <- function(x) {
  ...
  GLOBAL_envios <<- as.integer(x$envios / params$hyperparametertuning$xval_folds)   #asigno la variable global

  kfolds  <- params$hyperparametertuning$xval_folds   # cantidad de folds para cross validation
  ...
  param_basicos  <- list(objective = "binary",
                          ...
                          seed = params$hyperparametertuning$semilla_azar
                       )
  ...
  set.seed(params$hyperparametertuning$semilla_azar)
...
dataset  <- fread(params$input$dataset)

#creo la carpeta donde va el experimento
# HT  representa  Hiperparameter Tuning
dir.create("./exp/",  showWarnings = FALSE) 
dir.create(paste0("./exp/", params$experimento, "/"), showWarnings = FALSE)
setwd(paste0("./exp/", params$experimento, "/"))   #Establezco el Working Directory DEL EXPERIMENTO

#en estos archivos quedan los resultados
kbayesiana  <- paste0(params$experimento, ".RDATA")
klog        <- paste0(params$experimento, ".txt")
...
dataset[foto_mes %in% params$input$training, clase01 := ifelse(clase_ternaria == "CONTINUA", 0L, 1L)]


#los campos que se van a utilizar
campos_buenos  <- setdiff(colnames(dataset), c("clase_ternaria", "clase01", "azar", "training"))

set.seed(params$trainingstrategy$semilla_azar)
dataset[, azar := runif(nrow(dataset))]
dataset[, training := 0L]
dataset[foto_mes %in% params$input$training & 
          (azar <= params$trainingstrategy$undersampling | clase_ternaria %in% c("BAJA+1", "BAJA+2")),
         training := 1L]
...
ctrl  <- setMBOControlTermination(ctrl, iters = params$hyperparametertuning$iteraciones)   #cantidad de iteraciones
